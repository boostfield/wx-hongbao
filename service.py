...
def update_user(user):
    ...
    db.cursor().execute('UPDATE user SET openid=%s, agent=%s, share_qrcode=%s WHERE id=%s', args)
    db.commit()

def create_user(openid, agent=None):
    args = (openid, agent)
    db.cursor().execute('INSERT INTO user(openid, agent) VALUES(%s, %s)', args)
    db.commit()

def record_login(openid):
    args = (openid, )
    db.cursor().execute('INSERT INTO login(openid) VALUES(%s)', args)
    db.commit()

def save_user_pay(pay):
    args = (pay['openid'], pay['money'], pay['trade_no'], pay['ip'], pay['state'],
            pay.get('prepay_id'), pay.get('error_msg'))
    db.cursor().execute('INSERT INTO user_pay(openid, money, trade_no, ip, state, prepay_id, error_msg) VALUES(%s, %s, %s, %s, %s, %s, %s)', args)
    db.commit()
...
def update_user_pay(pay):
    ...
    db.cursor().execute('UPDATE user_pay SET openid=%s, money=%s, trade_no=%s, ip=%s, state=%s, prepay_id=%s, error_msg=%s WHERE id=%s', args)
    db.commit()
...
def update_sys_pay(pay):
    ...
    db.cursor().execute('UPDATE sys_pay SET openid=%s, money=%s, user_pay_id=%s, state=%s, type=%s, wx_billno=%s, error_msg=%s WHERE billno=%s', args)
...
def save_sys_pay(pay):
    ...
    db.cursor().execute('INSERT INTO sys_pay(openid, money, billno, user_pay_id, state, type, wx_billno, error_msg) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)', args)
    db.commit()
...
def save_event(openid, event, info=None):
    ...
    db.cursor().execute('INSERT INTO event(openid, type, info) VALUES(%s, %s, %s)', args)
    db.commit()
...
def save_share(uid, sid, money):
    args = (uid, sid, money)
    db.cursor().execute('insert into share(user_pay_id, sys_pay_id, money) values(%s, %s, %s)', args)
    db.commit()
